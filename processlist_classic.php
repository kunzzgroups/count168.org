<?php
/**
 * Process List 全页经典版（含 Games/Bank 与 POST 删除逻辑）。
 * 默认入口 `processlist.php` 302 到 React `/processlist`；嵌入模式加 `?c168_spa_frame=1`（无侧栏，供 SPA iframe）。
 */
if (!defined('PROCESSLIST_PAGE_FILE')) {
    define('PROCESSLIST_PAGE_FILE', basename($_SERVER['PHP_SELF'] ?? 'processlist_classic.php'));
}
...
$c168SpaFrame = isset($_GET['c168_spa_frame']) && $_GET['c168_spa_frame'] === '1';
...
if ($current_user_id && count($user_companies) > 0) {
    ...
    if (!$valid_company) {
        ...
        if (isset($_GET['company_id'])) {
            $redir = '?company_id=' . (int) $company_id;
            if (isset($_GET['search'])) {
                $redir .= '&search=' . urlencode($_GET['search']);
            }
            if ($c168SpaFrame) {
                $redir .= '&c168_spa_frame=1';
            }
...
$initialBankLayout = ($processListPageFile === 'bank_process_list.php')
    || (isset($_GET['category']) && strtoupper((string) $_GET['category']) === 'BANK');
?>

<!DOCTYPE html>
<html lang="en"<?php echo $c168SpaFrame ? ' data-c168-spa-frame="1"' : ''; ?>>
...
<body class="process-page<?php echo $initialBankLayout ? ' process-page--bank' : ''; ?><?php echo $c168SpaFrame ? ' process-list-spa-frame' : ''; ?>">
    <div class="container">
        <div class="content">
            <div
    ...
    <script>
        ...
        window.PROCESSLIST_COMPANY_CODE_BY_ID = <?php
            $codeById = [];
            if (!empty($user_companies)) {
                foreach ($user_companies as $c) {
                    if (isset($c['id'])) {
                        $codeById[(string) $c['id']] = $c['company_id'] ?? '';
                    }
                }
            }
            echo json_encode($codeById);
        ?>;
