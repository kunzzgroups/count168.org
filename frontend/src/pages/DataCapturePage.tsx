...
export default function DataCapturePage() {
  ...
  const [error, setError] = useState(''); const [date, setDate] = useState(today()); const [fetched, setFetched] = useState(false)

  const doFetch = async () => { setLoading(true); setError(''); try {
    const r = await api.get<{success:boolean;data:Row[]}>('/datacapture.php',{params:{date}})
  ...
  return (
    <ModuleSidebarLayout><div className="mx-auto w-full max-w-6xl space-y-6 py-4">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div>
        <h1 className="text-[clamp(2rem,0.625vw+1.2rem,2.4rem)] font-semibold text-zinc-900">Data Capture</h1>
        <p className="text-[clamp(1.2rem,0.15vw+1rem,1.3rem)] text-zinc-500">View and manage data capture records by date</p>
      </div><Button type="button" variant="outline" onClick={()=>navigate('/modules')}>Back to Modules</Button></header>
      <Card><CardHeader><CardTitle>Select Date</CardTitle></CardHeader><CardContent>
      ...
      {fetched && <Card><CardHeader><CardTitle>Captures ({items.length})</CardTitle></CardHeader><CardContent>
        <div className="overflow-x-auto"><table className="w-full min-w-[500px] border-collapse text-left text-sm"><thead><tr className="border-b border-zinc-200 text-zinc-600">
          ...
          {items.length===0?<tr><td colSpan={5} className="py-4 text-center text-zinc-500">No captures for this date.</td></tr>:
