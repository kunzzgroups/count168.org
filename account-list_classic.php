<?php
/**
 * Account List 经典全页（侧栏 + `js/account-list.js`）。
 * 默认入口 `account-list.php` 会 302 到 React `/accounts`；SPA 顶栏「经典版」指向本页。
 */
// 使用统一的session检查
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ids'])) {
    try {
        ...
        if (!empty($ids)) {
            ...
            if (!empty($activeAccounts)) {
                ...
                // 可以选择重定向到错误页面或显示错误消息
                header('Location: account-list_classic.php?error=cannot_delete_active');
                exit;
            }
            ...
            try {
                ...
                // 如果检查出错，为了安全起见，阻止删除
                header('Location: account-list_classic.php?error=delete_failed');
                exit;
            }
            ...
            if (!empty($accountsUsedInDatacapture)) {
                ...
                header('Location: account-list_classic.php?error=cannot_delete_used_in_datacapture&accounts=' . urlencode($usedAccountIds));
                exit;
            }
            ...
            // 重定向并带上成功消息
            header('Location: account-list_classic.php?deleted=' . $deletedCount);
            exit;
        }
    } catch (PDOException $e) {
        // 删除错误处理
    ...
    header('Location: account-list_classic.php');
    exit;
}
