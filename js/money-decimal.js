(function (global) {
    ...
    if (typeof global.Decimal === 'undefined') {
        throw new Error('decimal.js is required before money-decimal.js');
    }

    const Decimal = global.Decimal;
    ...
    function formatThousands(value, scale) {
    ...
    function add(a, b) { return toDecimal(a, 0).plus(toDecimal(b, 0)); }
    function sub(a, b) { return toDecimal(a, 0).minus(toDecimal(b, 0)); }
    function mul(a, b) { return toDecimal(a, 0).times(toDecimal(b, 0)); }
    function div(a, b) { return toDecimal(a, 0).div(toDecimal(b, 1)); }
    function abs(a) { return toDecimal(a, 0).abs(); }
    function max(a, b) { return Decimal.max(toDecimal(a, 0), toDecimal(b, 0)); }
    function min(a, b) { return Decimal.min(toDecimal(a, 0), toDecimal(b, 0)); }
    function cmp(a, b) { return toDecimal(a, 0).cmp(toDecimal(b, 0)); }
